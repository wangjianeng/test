...
class InboxController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('inbox/index',['users'=>$this->getUsers()]);
    ...
    public function show($id)
    {
        ...
        return view('inbox/view',['email_history'=>$email_history,'order'=>$order,'email'=>$email,'users'=>$this->getUsers(),'accounts'=>$this->getAccounts()]);
    }
   ...
   */
        ...
        for($i = $iDisplayStart; $i < $end; $i++) {
            ...
            $records["data"][] = array(
                ...
                (($customersList[$i]['etype'])?'<span class="label label-sm label-danger">'.$customersList[$i]['etype'].'</span> ':'').'<a href="/inbox/'.$customersList[$i]['id'].'" target="_blank" style="color:#333;">'.(($customersList[$i]['read'])?'':'<strong>').$customersList[$i]['subject'].(($customersList[$i]['read'])?'':'</strong>').'</a>'.(($warnText)?'<span class="label label-sm label-danger">'.$warnText.'</span> ':'').(($customersList[$i]['remark'])?'<BR/><span class="label label-sm label-warning">'.$customersList[$i]['remark'].'</span> ':''),
                $customersList[$i]['date'],
                $status_list[$customersList[$i]['reply']],
                $users[$customersList[$i]['user_id']],
                '<a href="/inbox/'.$customersList[$i]['id'].'" class="btn btn-sm btn-outline grey-salsa"><i class="fa fa-search"></i> View </a>',
            );
        }
    ...
    public function getAccounts(){
        ...
        foreach($accounts as $account){
            $accounts_array[$account['account_email']] = $account['account_name'];
        }
