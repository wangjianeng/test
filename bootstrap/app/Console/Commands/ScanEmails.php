...
class ScanEmails extends Command
{
     ...
     * @var string
     */
    protected $signature = 'scan:email';
    ...
    public function handle()
    {
        ...
        $this->account_email = array();
        foreach($accs as $account){
            $this->account_email[strtolower($account->account_email)] = $account->account_sellerid;
        }

        $accounts = DB::table('accounts')->groupBy('email','password','imap_host','imap_ssl','imap_port');
        $accountList = $accounts->get(array('email','password','imap_host','imap_ssl','imap_port'));
    ...
    public function saveEmails(){
        $date = date('j F Y',strtotime('- 30day'));
        $attDir = public_path('attachs').'/'.date('Ymd');
        if (!is_dir($attDir)) mkdir($attDir, 0777,true);
		Log::Info(' '.$this->runAccount['email'].' Since '.$date.' Emails Scan Start...');
        ...
        $mailsIds = $mailbox->searchMailbox('ALL'); //TO 参数不起作用 没办法，下面循环比较
        if(!$mailsIds) {
            Log::Info(' '.$this->runAccount['email'].' Since '.$date.' Mailbox is empty...');
        }else{
            foreach($mailsIds as $mailsId){
                ...
                if(!$exists) {
                    try{
                        ...
                        if(!$mail->to) continue;
                        $this->runAccount['account_email']='';
                        foreach($mail->to as $k=>$v){
                            if(array_key_exists(strtolower($k),$this->account_email)){
                                $this->runAccount['account_email']=strtolower($k);
                                $this->runAccount['account_sellerid']=$this->account_email[strtolower($k)];
                                break;
                            }
                        }
                        if(!$this->runAccount['account_email']){
                        ...
                        if($mail){
                            ...
                            Log::Info(' '.$this->runAccount['email'].' MailID '.$mailsId.' Insert Success...');
                        }
                    }catch (\Exception $e){
                        Log::Info(' '.$this->runAccount['email'].' MailID '.$mailsId.' Insert Error...'.$e->getMessage());
                    }
                }else{
                    Log::Info(' '.$this->runAccount['email'].' MailID '.$mailsId.' AlReady Exists...');
                }
            }
        }
        Log::Info(' '.$this->runAccount['email'].' Since '.$date.' Emails Scan Complete...');
    }
