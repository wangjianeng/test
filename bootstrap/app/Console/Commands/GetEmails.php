...
class GetEmails extends Command
{
    ...
    public function saveEmails($time){
        $lastUser = DB::table('inbox')->orderBy('date','desc')->first();
        print_r($lastUser->user_id);
        die();
        ...
        $mailbox = new PhpImap\Mailbox('{'.$this->runAccount['imap_host'].':'.$this->runAccount['imap_port'].'/imap/'.$this->runAccount['imap_ssl'].'}', $this->runAccount['email'], $this->runAccount['password'], $attDir);
        $boxes = $mailbox->getMailboxes();
        foreach($boxes as $k=>$v ){
            if(in_array($v['shortpath'],array('&UXZO1mWHTvZZOQ-','Sent Messages','Drafts','Deleted Messages','Deleted Messages','Junk'))) continue;
            $mailbox1 = new PhpImap\Mailbox($v['fullpath'], $this->runAccount['email'], $this->runAccount['password'], $attDir);
            $mailsIds = $mailbox1->searchMailbox('UNFLAGGED');
            foreach($mailsIds as $mailsId){
                $mail = $mailbox->getMail($mailsId);
                print_r($mail);

            }
            $mailbox1->setFlag($mailsIds,'\\Flagged');
        }
        die();

        $mbox = new PhpImap\Mailbox('{'.$this->runAccount['imap_host'].':'.$this->runAccount['imap_port'].'/imap/'.$this->runAccount['imap_ssl'].'}', $this->runAccount['email'], $this->runAccount['password'], $attDir);

        $boxes = $mbox->getMailboxes();
        if (is_array($boxes)) {
            foreach ($boxes as $key => $val) {
                $mailbox = new PhpImap\Mailbox($val['fullpath'], $this->runAccount['email'], $this->runAccount['password'], $attDir);
            ...
            Log::Info("imap_getmailboxes failed: " . imap_last_error());
        }
